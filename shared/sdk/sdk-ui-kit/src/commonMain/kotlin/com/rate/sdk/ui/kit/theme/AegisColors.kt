 ...
 * Aegis colour tokens.
 *
 * A [AegisColorTokens] data class with two canonical instances ([LightColors],
 * [DarkColors]) selected by [AegisTheme] and threaded through [LocalAegisColors].
 *
 ...
 * so cards still feel raised. Brand stays #E31837-adjacent so it pops against the
 * dark canvas without the eye-watering saturation of the pure indigo. The slate
 * ramp is inverted (slate1 = darkest canvas, slate12 = lightest inky-replacement)
 * so semantic aliases (textBody = slate9, canvas = slate1, etc.) keep working
 * unchanged.
 *
 ...
 */
val LocalAegisColors = staticCompositionLocalOf { LightColors }
