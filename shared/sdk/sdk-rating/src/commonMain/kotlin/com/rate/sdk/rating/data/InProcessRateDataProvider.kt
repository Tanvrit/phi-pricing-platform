...
/**
 * In-process rate provider that runs without a server. Used by the WASM customer journey
 * (no Mongo in the browser) and as an offline fallback for the desktop operator console.
 *
 * Relocated VERBATIM from the monolith's `com.rate.domain.data.InProcessRateDataProvider`
 * (imports repackaged to core's [RateDataProvider]/[Plan]/etc. and this module's [CoverIds]).
 * Numbers are plausibly-scaled but ILLUSTRATIVE, not actuarial — the Mongo-backed
 * `RateDataProvider` in server-persistence is the production source of truth. This provider
 * exists so the *engine path* is exercised consistently (no special-case "client-only fake
 * pricing") even when the backend isn't reachable.
 *
 * Lifecycle: replace with a network-cached snapshot of the real rate table the first time the
 * customer's browser successfully talks to the catalog/rating API.
 */
class InProcessRateDataProvider : RateDataProvider {
     ...
     * Same shape as Excel's `(age band, SI) → base premium` table; numbers are roughly
     * tier × age × SI-bucket scaled so the headline quote is in the right order of magnitude
     * (₹X,000 – ₹X,00,000/year for retail PHI).
     */
...
/**
 * In-process GROUP rate provider — extends [InProcessRateDataProvider] with the two group
 * factors so the [com.rate.sdk.rating.handler.GroupPricingEngine] can run offline too. Volume
 * discount grows with head-count (capped); industry loading is a small illustrative table.
 */
class InProcessGroupRateDataProvider :
    com.rate.core.rating.ports.GroupRateDataProvider,
    com.rate.core.rating.ports.RateDataProvider by InProcessRateDataProvider() {
    ...
    private companion object {
        /** Illustrative industry risk loadings (mining/heavy-industry > IT/services). */
