 ...
 *
 * No axes, dots, or per-point hover — this is a pure visual hint showing the
 * shape of the last-N-buckets trend so an operator can scan the KPI strip and
 * see at a glance whether things are climbing, sliding, or flat.
 *
 * When `labels` is provided (and matches `values.size`), a Material3 tooltip
 * shows the count of points + the latest label on hover/long-press. This is
 * the simpler "single-tooltip" path — operators get the freshest data point
 * without the complexity of cursor tracking. A mismatched `labels` list falls
 * back to stringifying the last value.
 *
 * Empty / single-value series fall back to a flat horizontal mid-line so the
 * tile still shows something rather than a blank gap.
 */
