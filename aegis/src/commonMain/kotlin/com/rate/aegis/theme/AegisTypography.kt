 ...
 *
 * We use system fonts for now (Inter / JetBrains Mono are intended; we fall back to
 * the platform's default sans + monospace so the gallery launches without bundled
 * font assets). Money / numeric styles use `FontFamily.Monospace` and request
 * tabular figures via `fontFeatureSettings = "tnum"` so digits align in tables.
 */
